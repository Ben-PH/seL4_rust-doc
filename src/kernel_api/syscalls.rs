...
pub fn send(reciever: CapPtr, msg: MessageInfo) -> () {unimplemented!()}
/// Polling send on an endpoint
///
/// like `send`, with key difference:
///  * instead of a block, message will silently drop
pub fn nb_send(reciever: CapPtr, msg: MessageInfo) -> () {unimplemented!()}
...
pub fn call(invoked_cap: CapPtr, info: MessageInfo) -> MessageInfo {unimplemented!()}

/// Uses reply cap generated by `call()` to respond to it.
///
/// Same behaviour as using the cap with `send()`(sel4 manual: 4.2.4)
pub fn reply(msg: MessageInfo) -> () {unimplemented!()}
